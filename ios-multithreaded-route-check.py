...
def main(argv):
    ...
    if len(argv) < 4:
        ...
        if (argv[2].upper() == 'ALL') and (argv[1].upper() != 'FILE'):
            ...
            if 'no routing information' in result.lower() or 'exception' in result.lower():
...
def help_and_exit(prog):
    ...
    print('Usage: ' + prog + ' FILE, there should be an inventory.csv file in the same folder as the script')
    sys.exit(1)
...
def get_route_table(wsds_device, user, passwd):
    ...
    device = driver(hostname=wsds_device, username=user,
                    password=passwd)  # optional_args = {'port': 8181}
    try:
        device.open()
...
def get_route(wsds_device, destination, user, passwd):
    ...
    if str(regresult) == 'None':
        ...
        try:
            device.open()
            route_details = device.get_route_to(destination)
            device.close()
            ...
            if len(route_details) == 0:
                ...
                for route_key in route_details:
                    ...
                    return json.dumps(route_details, sort_keys=True, indent=4)
            else:
                return json.dumps(route_details, sort_keys=True, indent=4)
        except:
            # TODO: Exception handling should/could be narrowed down to specific exceptions
